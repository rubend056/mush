...
The row tables still owe five items; two §6 notes and the structural queue in
`docs/refactor.md` sit beside them. This is the one list: each row carries what
the defect costs and where the fix belongs, and a closure is recorded on the
row, never here.

- **H6** — the load-sensitive 60fps frame test; the `Clock`/`Machine`/`Events`
  seams exist so it can become a test of an idle box (or be `#[ignore]`d).
...
- **H12** — per-agent token accounting, so a run's cost is visible while it is
  spent.
- **V7** — the sweep's named blind spots: hidden-row counts, tier boundaries
  and window/count drift.
- **A7** — fixed, but its test is still owed: a thread-spawn failure is not
  scriptable here.

The refactor queue's `D9`, `D10`, `R9`, `R10`'s two stale comments and
`R26`–`R29` are structural, not defects; `docs/refactor.md` §11 is their ledger
and carries their prices.

Two further open items are stated in §6, both interactions with H9: an attach
op's `say` (a `focus`, or an `edit`'s draft) silently disarms an armed quit, and
`Phase::doing` reads a stopped agent that owns a job as `idle`. They belong with
whoever next touches the quit line.
...
| V6 | **The bar's sanitize invariant is documented as total but not held.** `app/mod.rs` says "every string the bar can show is created by `say`/`fail`", while `bar_word(self.status_line(), tree.as_deref())` lets `tree_line`'s string reach the bar *outside* `set_status`'s sanitize door. Harmless today (numbers and fixed words) and fragile the moment an agent label lands in that sentence. Part of `R10`. | ✅ | `tree_line` sanitizes its sentence before the bar sees it, so the door is total (`f70374f`); the remaining stale bar comments are refactor `R10` |
