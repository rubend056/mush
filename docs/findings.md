...
| ✅ | closed in `e747bc3`: the empty transcript says the same refusal as the short one (`a_fold_of_an_empty_transcript_says_so_instead_of_nothing`), and the automatic trigger is still told nothing |
| ✅ | closed in `e747bc3` for the notice half: `isolated unavailable: not a git repository — it shares this workspace` reaches the parent's pane while the row carries no branch (`a_degraded_isolation_is_said_to_the_human_too`). **The docs half is still open** — `docs/mush.md` §5.5 says only "an `isolated` one inside its own git worktree" and never names the in-place fallback |
