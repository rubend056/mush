...
| `search` | `pattern`, `path?` | a Rust regex (`regex-lite`) matched against each line — so `^`/`$` anchor a line and a `\n` in the pattern never matches — one `path:line: text` per match; `(?i)` is how case-insensitive is said and a metacharacter meant literally is escaped (`foo\(bar\)`); no lookaround or backreference, no `\p{…}` classes, and `\w`/`\b` are ASCII-only; binary and huge files skipped |
