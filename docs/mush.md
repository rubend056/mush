...
| `read_file` | `path`, `offset?`, `limit?` | a file as a window of lines, with no line numbers, and one trailing sentence saying what the window left; an unbounded read — `offset` and `limit` both absent — of a file longer than the result cap answers with the file's outline instead of a cut head, and names the `offset`/`limit` range that reads on; a png, jpeg, gif or webp — sniffed from the file's own bytes, never its name — comes back as the image itself, if the model is documented to see; works beside a held lock |
| `outline` | `path` | the definitions in one file, one row per declaration — `line  <that line, cut to a width>` — so a file's shape costs a screen instead of a window; textual and best-effort across the languages a source file is likely to be written in (Rust, Go, Python, JavaScript and TypeScript, C and C++, C#, Java, Swift, Kotlin, Ruby, PHP, Scala, Haskell, SQL, and the rest — it is not a compiler: macro-generated items are invisible, and a Java method whose return type is not `void`, a C `int main(void)`, a shell `build() {` and a Makefile `build:` are misses), and each row is the anchor `read_file {offset}` and `edit_file {old_string}` are built from; a file with no definitions answers with a sentence, never a refusal |
...
| `search` | `pattern`, `path?`, `context?` | a Rust regex (`regex-lite`) matched against each line — so `^`/`$` anchor a line and a `\n` in the pattern never matches — one `path:line: text` per match; `context` (default 0) adds up to ten lines either side of each match, like `rg -C` — the neighbours are `path-line- text` rows, overlapping windows merge into one run, a window stops at the file's first and last line, and a bigger ask is clamped to the ten that fit, not refused — and the cap counts rows, so `more` still means one row was left; `(?i)` is how case-insensitive is said and a metacharacter meant literally is escaped (`foo\(bar\)`); no lookaround or backreference, no `\p{…}` classes, `\w`/`\b` are ASCII-only, and `(?i)`'s fold is too (`é` does not match `É`); binary and huge files skipped |
| `usages` | `symbol` | who mentions a symbol as a *word* — alphanumerics and `_` are the word characters, so `held` in `beheld` is not a row — grouped by file, each group's header counting its hits and naming the declaration-looking line (the outline rule's) first; textual and best-effort, with no identifier resolution, no scope and no call graph (a `.` or `::` does not split a needle, so `method` hits `self.method()` and `Type::method`; a comment or a string can be a row), and a miss says how many files were read and counts the ones it could not open, so it never reads as "the symbol is not here"; `path` is deliberately absent — `search { pattern, path }` is the tool that narrows a file — and no word toggle exists anywhere: the word is the rule above, and `\b` semantics are written `\b`, ASCII-only as the `search` row states |
...
search's `in crates`, a command's arguments, those flags. A call
