...
impl Drop for Foreground {
    ...
    /// process group id may be free to be handed to somebody else's process,
    /// and a `kill -9 -pgid` that landed there would kill work mush never
    /// started. What the group of such a command needs is taken the moment the
    /// end is seen, while the id is still provably its own
    /// ([`Foreground::poll`]); what keeps a *running* command from escaping is
    /// that the entry is registered for the whole of the call, not this drop.
    fn drop(&mut self) {
